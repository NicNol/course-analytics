...
const CourseTableHeader: FC<CourseTableHeaderProps> = ({ columns, setColumnState }) => {
  ...
  return (
    <Table.Header>
      <Table.Row bg={"colorPalette.muted"}>
        {columns.map((column) => (
          <CourseTableHeaderCell key={column.accessor} column={column} handleHeaderClick={handleHeaderClick} />
        ))}
      </Table.Row>
    </Table.Header>
  );
