...
async function prepareSheet(): Promise<any> {
  try {
    ...
    newSpreadsheet.sheetsByIndex.forEach(async (sheet, index) => {
      if (index > 0) await sheet.delete();
    });
    ...
    await originalSpreadsheet.sheetsByIndex[0].copyToSpreadsheet(newSpreadsheetID);

    /* Reload newSpreadsheet data to prevent errors */
    await newSpreadsheet.loadInfo();
    const newSheet: GoogleSpreadsheetWorksheet = newSpreadsheet.sheetsByIndex[1];
